...
def increment_user_vector(customer_baskets: Dict[str, list],
    ...
    if IS_DEC:
        checkpoint = f"{checkpoint}/dec_1k{inner_path}"
    else:
    ...
    for customer_id in tqdm.tqdm(customer_ids, desc="Creating user vector"):
        ...
        if IS_DEC and customer_id in remove_basket_customer_ids:
            ...
            if remove_count >= 1:
                ...
                temp = [list_of_basket_vetorized[i] for i in range(len(list_of_basket_vetorized)) if i not in remove_indice]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--basket_decay_rate", default=1.0, type=float, help="time decay ratio within a group")
    parser.add_argument("--group_decay_rate", default=0.6, type=float, help="time decay ratio across groups")
    parser.add_argument("--data_file", help="historical baskets", default="./data/VS_history_order.csv")
    parser.add_argument("--group_size", default=7, type=int, help="the size of a group")
    parser.add_argument("--checkpoint", default="VS_checkpoint", help="checkpoint path")
    parser.add_argument("--clear_state", default=False, type=bool, help="clear path")
    parser.add_argument("--use_external_vocab", default=False, type=bool, help="for single file which does not contain all")
